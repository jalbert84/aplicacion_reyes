...
// Obtener los usuarios de la calle Plaza9dOctubre
$usuariosController = new UsuariosController();
$usuariosPlaza9dOctubre = $usuariosController->obtenerUsuariosPorCalleController("plaza_nou_doctubre");

$orden = 1;
foreach ($usuariosPlaza9dOctubre as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'plazanoudoctubre');
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Plaça 9 d'Octubre</h1>
    ...
    <?php if (!empty($usuariosPlaza9dOctubre)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosPlaza9dOctubre);
        
        // Reiniciar el puntero del array
        reset($usuariosPlaza9dOctubre);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosPlaza9dOctubre) - 1; $i++) : 
            $usuario = $usuariosPlaza9dOctubre[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosPlaza9dOctubre); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle Plaza9dOctubre.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [8, 9, 2];
