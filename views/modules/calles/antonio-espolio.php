...
// Obtener los usuarios de la calle AntonioEspolio
$usuariosController = new UsuariosController();
$usuariosAntonioEspolio = $usuariosController->obtenerUsuariosPorCalleController("antonio_espolio");

$orden = 1;
foreach ($usuariosAntonioEspolio as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'antonioespolio');
...
<head>
    ...
    <title>Carrer Antonio Espolio</title>
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Antonio Espolio</h1>
    ...
    <div id="usuarios" class="usuarios-container">
        ...
        $ultimoUsuario = end($usuariosAntonioEspolio);
        
        // Reiniciar el puntero del array
        reset($usuariosAntonioEspolio);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosAntonioEspolio) - 1; $i++) : 
            $usuario = $usuariosAntonioEspolio[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosAntonioEspolio); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle AntonioEspolio.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [8, 6, 4, 2,];
