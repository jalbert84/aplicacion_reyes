...
// Obtener los usuarios de la calle MestreSerrano
$usuariosController = new UsuariosController();
$usuariosMestreSerrano = $usuariosController->obtenerUsuariosPorCalleController("mestre_serrano");

$orden = 1;
foreach ($usuariosMestreSerrano as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'mestreserrano');
...
<head>
    ...
    <title>Carrer Mestre Serrano</title>
    ...
    .boton-container, .actions-container, .no-print, .btn-mapa-gris, .btn-primary, .remove-btn {
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Mestre Serrano</h1>
    ...
    <div id="usuarios" class="usuarios-container">
        ...
        $ultimoUsuario = end($usuariosMestreSerrano);
        
        // Reiniciar el puntero del array
        reset($usuariosMestreSerrano);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosMestreSerrano) - 1; $i++) : 
            $usuario = $usuariosMestreSerrano[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosMestreSerrano); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle MestreSerrano.</p>
<?php endif; ?>
