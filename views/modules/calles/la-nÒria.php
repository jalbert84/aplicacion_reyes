...
// Obtener los usuarios de la calle LaNoria
$usuariosController = new UsuariosController();
$usuariosLaNoria = $usuariosController->obtenerUsuariosPorCalleController("la_noria");

$orden = 1;
foreach ($usuariosLaNoria as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'lanoria');
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer La Nòria</h1>
    ...
    <?php if (!empty($usuariosLaNoria)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosLaNoria);
        
        // Reiniciar el puntero del array
        reset($usuariosLaNoria);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosLaNoria) - 1; $i++) : 
            $usuario = $usuariosLaNoria[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosLaNoria); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle LaNoria.</p>
<?php endif; ?>
