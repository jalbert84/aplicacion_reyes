...
// Obtener los usuarios de la calle SantBertomeu
$usuariosController = new UsuariosController();
$usuariosSantBertomeu = $usuariosController->obtenerUsuariosPorCalleController("sant_bertomeu");

$orden = 1;
foreach ($usuariosSantBertomeu as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'santbertomeu');
...
<head>
    ...
    <title>Carrer Sant Bertomeu</title>
    ...
    .boton-container, .actions-container, .no-print, .btn-mapa-rojo, .btn-primary, .remove-btn {
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Sant Bertomeu</h1>
    ...
    <?php if (!empty($usuariosSantBertomeu)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosSantBertomeu);
        
        // Reiniciar el puntero del array
        reset($usuariosSantBertomeu);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosSantBertomeu) - 1; $i++) : 
            $usuario = $usuariosSantBertomeu[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosSantBertomeu); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle SantBertomeu.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [2, 4, 3, 6, 8, 10, 5, 12, 7, 16, 9, 18, 20, 22, 24, 26, 17, 30, 32, 19, 21, 36, 38, 40, 42, 31, 46, 48, 52, 33, 54, 35, 58, 37, 60, 39, 62, 41, 64, 43, 66, 68, 45, 70, 47, 72, 74, 76, 78, 80, 84];
