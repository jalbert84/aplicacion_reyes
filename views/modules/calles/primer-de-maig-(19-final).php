...
// Obtener los usuarios de la calle PrimerDeMaig19Final
$usuariosController = new UsuariosController();
$usuariosPrimerDeMaig19Final = $usuariosController->obtenerUsuariosPorCalleController("primer_de_maig_19_final");

$orden = 1;
foreach ($usuariosPrimerDeMaig19Final as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'primerdemaig19final');
...
<body>
    ...
    <?php if (!empty($usuariosPrimerDeMaig19Final)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosPrimerDeMaig19Final);
        
        // Reiniciar el puntero del array
        reset($usuariosPrimerDeMaig19Final);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosPrimerDeMaig19Final) - 1; $i++) : 
            $usuario = $usuariosPrimerDeMaig19Final[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosPrimerDeMaig19Final); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle PrimerDeMaig19Final.</p>
<?php endif; ?>
...
function guardarOrdenUsuarios() {
    ...
    fetch('./primer-de-maig-(19-final).php', {
        method: 'POST',
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [19, 20, 21, 22, 34, 35, 36, 37, 38, 39, 40, 41, 42];
