...
// Obtener los usuarios de la calle Assegadors
$usuariosController = new UsuariosController();
$usuariosAssegadors = $usuariosController->obtenerUsuariosPorCalleController("assegadors");

$orden = 1;
foreach ($usuariosAssegadors as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'assegadors');
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Assegadors</h1>
    ...
    <?php if (!empty($usuariosAssegadors)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosAssegadors);
        
        // Reiniciar el puntero del array
        reset($usuariosAssegadors);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosAssegadors) - 1; $i++) : 
            $usuario = $usuariosAssegadors[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosAssegadors); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle Assegadors.</p>
<?php endif; ?>
