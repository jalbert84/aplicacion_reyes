...
// Obtener los usuarios de la calle TirantLoBlanc
$usuariosController = new UsuariosController();
$usuariosTirantLoBlanc = $usuariosController->obtenerUsuariosPorCalleController("tirant_lo_blanc");

$orden = 1;
foreach ($usuariosTirantLoBlanc as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'tirantloblanc');
...
<head>
    ...
    <title>Carrer Tirant lo Blanc</title>
...
<body>
    ...
    <div id="usuarios" class="usuarios-container">
        ...
        $ultimoUsuario = end($usuariosTirantLoBlanc);
        
        // Reiniciar el puntero del array
        reset($usuariosTirantLoBlanc);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosTirantLoBlanc) - 1; $i++) : 
            $usuario = $usuariosTirantLoBlanc[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosTirantLoBlanc); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle TirantLoBlanc.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [21, 19, 17, 15, 13, 11, 9];
