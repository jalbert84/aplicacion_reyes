...
// Obtener los usuarios de la calle LaSequia
$usuariosController = new UsuariosController();
$usuariosLaSequia = $usuariosController->obtenerUsuariosPorCalleController("la_sequia");

$orden = 1;
foreach ($usuariosLaSequia as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'lasequia');
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer La Sèquia</h1>
    ...
    <?php if (!empty($usuariosLaSequia)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosLaSequia);
        
        // Reiniciar el puntero del array
        reset($usuariosLaSequia);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosLaSequia) - 1; $i++) : 
            $usuario = $usuariosLaSequia[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosLaSequia); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle LaSequia.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [1, 2, 4, 6, 8, 10, 3, 5, 7, 12, 13, 14, 9];
