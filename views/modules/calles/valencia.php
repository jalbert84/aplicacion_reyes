...
// Obtener los usuarios de la calle Valencia
$usuariosController = new UsuariosController();
$usuariosValencia = $usuariosController->obtenerUsuariosPorCalleController("Valencia");

$orden = 1;
foreach ($usuariosValencia as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'valencia');
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer València</h1>
    ...
    <?php if (!empty($usuariosValencia)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosValencia);
        
        // Reiniciar el puntero del array
        reset($usuariosValencia);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosValencia) - 1; $i++) : 
            $usuario = $usuariosValencia[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosValencia); ?></div>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [2, 1, 4, 3, 6, 5, 7, 8, 10, 9, 11, 12, 13, 14];
