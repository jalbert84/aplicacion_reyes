...
// Obtener los usuarios de la calle MestrePalau
$usuariosController = new UsuariosController();
$usuariosMestrePalau = $usuariosController->obtenerUsuariosPorCalleController("mestre_palau");

$orden = 1;
foreach ($usuariosMestrePalau as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'mestrepalau');
...
<head>
    ...
    <title>Carrer Mestre Palau</title>
...
<body>
    ...
    <div id="usuarios" class="usuarios-container">
        ...
        $ultimoUsuario = end($usuariosMestrePalau);
        
        // Reiniciar el puntero del array
        reset($usuariosMestrePalau);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosMestrePalau) - 1; $i++) : 
            $usuario = $usuariosMestrePalau[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosMestrePalau); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle MestrePalau.</p>
<?php endif; ?>
