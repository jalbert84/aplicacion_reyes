...
// Obtener los usuarios de la calle AusiasMarch
$usuariosController = new UsuariosController();
$usuariosAusiasMarch = $usuariosController->obtenerUsuariosPorCalleController("ausias_march");

$orden = 1;
foreach ($usuariosAusiasMarch as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'ausiasmarch');
...
<head>
    ...
    <title>Carrer Ausiàs March</title>
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Ausiàs March</h1>
    ...
    <?php if (!empty($usuariosAusiasMarch)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosAusiasMarch);
        
        // Reiniciar el puntero del array
        reset($usuariosAusiasMarch);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosAusiasMarch) - 1; $i++) : 
            $usuario = $usuariosAusiasMarch[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosAusiasMarch); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle AusiasMarch.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [13, 11, 9, 7, 5, 6, 4, 3, 2];
