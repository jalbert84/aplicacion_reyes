...
// Obtener los usuarios de la calle DonEmilioRamonLlin
$usuariosController = new UsuariosController();
$usuariosDonEmilioRamonLlin = $usuariosController->obtenerUsuariosPorCalleController("don_emilio_ramon-llin");

$orden = 1;
foreach ($usuariosDonEmilioRamonLlin as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'donemilioramonllin');
...
<head>
    ...
    <title>Carrer Don Emilio Ramón Llin</title>
    ...
    .boton-container, .actions-container, .no-print, .btn-mapa-verde, .btn-primary, .remove-btn {
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Don Emilio Ramón Llin</h1>
    ...
    <?php if (!empty($usuariosDonEmilioRamonLlin)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosDonEmilioRamonLlin);
        
        // Reiniciar el puntero del array
        reset($usuariosDonEmilioRamonLlin);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosDonEmilioRamonLlin) - 1; $i++) : 
            $usuario = $usuariosDonEmilioRamonLlin[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosDonEmilioRamonLlin); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle DonEmilioRamonLlin.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [34, 51, 49, 32, 45, 43, 41, 39, 37, 35, 33, 31, 28, 29, 27, 26, 25, 24, 23, 22, 21, 19, 20, 18, 17, 15, 16, 13, 14, 11, 12, 10, 9, 7, 8, 5, 6, 3, 4, 1, 2];
