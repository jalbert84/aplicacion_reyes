...
// Obtener los usuarios de la calle RamonICajal
$usuariosController = new UsuariosController();
$usuariosRamonICajal = $usuariosController->obtenerUsuariosPorCalleController("ramon_i_cajal");

$orden = 1;
foreach ($usuariosRamonICajal as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'ramonicajal');
...
<head>
    ...
    <title>Carrer Ramón i Cajal</title>
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Ramón i Cajal</h1>
    ...
    <?php if (!empty($usuariosRamonICajal)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosRamonICajal);
        
        // Reiniciar el puntero del array
        reset($usuariosRamonICajal);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosRamonICajal) - 1; $i++) : 
            $usuario = $usuariosRamonICajal[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosRamonICajal); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle RamonICajal.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [10, 8, 4];
