...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'tarazona');
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Tarazona</h1>
    ...
    <div id="usuarios" class="usuarios-container">
        ...
        $ultimoUsuario = end($usuariosTarazona);
        
        // Reiniciar el puntero del array
        reset($usuariosTarazona);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosTarazona) - 1; $i++) : 
            $usuario = $usuariosTarazona[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosTarazona); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle Tarazona.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [6, 4, 2];
