...
// Obtener los usuarios de la calle Calvari
$usuariosController = new UsuariosController();
$usuariosCalvari = $usuariosController->obtenerUsuariosPorCalleController("calvari");

$orden = 1;
foreach ($usuariosCalvari as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'calvari');
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Calvari</h1>
    ...
    <?php if (!empty($usuariosCalvari)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosCalvari);
        
        // Reiniciar el puntero del array
        reset($usuariosCalvari);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosCalvari) - 1; $i++) : 
            $usuario = $usuariosCalvari[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosCalvari); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle Calvari.</p>
<?php endif; ?>
