...
// Obtener los usuarios de la calle Puntarro
$usuariosController = new UsuariosController();
$usuariosPuntarro = $usuariosController->obtenerUsuariosPorCalleController("puntarro");

$orden = 1;
foreach ($usuariosPuntarro as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'puntarro');
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Puntarró</h1>
    ...
    <?php if (!empty($usuariosPuntarro)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosPuntarro);
        
        // Reiniciar el puntero del array
        reset($usuariosPuntarro);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosPuntarro) - 1; $i++) : 
            $usuario = $usuariosPuntarro[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosPuntarro); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle Puntarro.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [1, 3, 5, 16, 7, 34, 36, 42, 44, 9, 46];
