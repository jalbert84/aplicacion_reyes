...
// Obtener los usuarios de la calle Rajolars
$usuariosController = new UsuariosController();
$usuariosRajolars = $usuariosController->obtenerUsuariosPorCalleController("rajolars");

$orden = 1;
foreach ($usuariosRajolars as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'rajolars');
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Rajolars</h1>
    ...
    <div id="usuarios" class="usuarios-container">
        ...
        $ultimoUsuario = end($usuariosRajolars);
        
        // Reiniciar el puntero del array
        reset($usuariosRajolars);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosRajolars) - 1; $i++) : 
            $usuario = $usuariosRajolars[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosRajolars); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle Rajolars.</p>
<?php endif; ?>
