...
// Obtener los usuarios de la calle SantaBarbera
$usuariosController = new UsuariosController();
$usuariosSantaBarbera = $usuariosController->obtenerUsuariosPorCalleController("santa_barbera");

$orden = 1;
foreach ($usuariosSantaBarbera as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'santabarbera');
...
<body>
    ...
    <?php if (!empty($usuariosSantaBarbera)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosSantaBarbera);
        
        // Reiniciar el puntero del array
        reset($usuariosSantaBarbera);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosSantaBarbera) - 1; $i++) : 
            $usuario = $usuariosSantaBarbera[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosSantaBarbera); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle SantaBarbera.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [47, 44, 45, 42, 43, 40, 38, 41, 39, 36, 37, 35, 33, 34, 31, 29, 28, 27, 26, 25, 24, 23, 21, 19, 18, 17, 15, 16, 13, 14, 11, 12, 9, 7, 10, 8, 5, 6, 4, 2];
