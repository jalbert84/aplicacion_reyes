...
// Obtener los usuarios de la calle VicentLassala
$usuariosController = new UsuariosController();
$usuariosVicentLassala = $usuariosController->obtenerUsuariosPorCalleController("vicent_lassala");

$orden = 1;
foreach ($usuariosVicentLassala as &$usuario) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['usuariosOrdenados'])) {
        ...
        try {
            $mensaje = $usuariosController->actualizarOrdenUsuariosController($usuariosOrdenados, 'vicentlassala');
...
<head>
    ...
    <title>Carrer Vicent Lassala</title>
...
<body>
    ...
    <h1 class="text-center" style="color: white;">Carrer Vicent Lassala</h1>
    ...
    <?php if (!empty($usuariosVicentLassala)) : ?>
    <div id="usuarios" class="usuarios-container">
        <?php 
        // Obtener el último usuario
        $ultimoUsuario = end($usuariosVicentLassala);
        
        // Reiniciar el puntero del array
        reset($usuariosVicentLassala);
        
        // Mostrar todos los usuarios excepto el último
        for ($i = 0; $i < count($usuariosVicentLassala) - 1; $i++) : 
            $usuario = $usuariosVicentLassala[$i];
        ?>
        ...
        <div class="usuario" data-id="<?php echo $ultimoUsuario['id']; ?>">
            <div><?php echo count($usuariosVicentLassala); ?></div>
...
<?php else : ?>
    <p>No se encontraron usuarios registrados en la calle VicentLassala.</p>
<?php endif; ?>
...
function ordenarAutomaticamente() {
    ...
    // Orden personalizado
    const ordenPersonalizado = [3, 5, 1];
